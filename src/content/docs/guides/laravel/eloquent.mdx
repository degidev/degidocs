...
Los modelos Eloquent te permiten consultar datos en tus tablas, así como insertar, actualizar y eliminar registros de esas tablas sin escribir SQL manualmente.

## Creación de modelos

Los modelos en Laravel se almacenan en el directorio <Code code="app/Models" lang="php" inline /> y extienden la clase <Code code="Illuminate\Database\Eloquent\Model" lang="php" inline />.

### Generando modelos con Artisan

La forma más rápida de crear un modelo es utilizando el comando Artisan <Code code="make:model" lang="php" inline />:

<Code code="php artisan make:model Producto" lang="bash" />

Este comando generará una clase modelo en <Code code="app/Models/Producto.php" lang="php" inline />.

### Opciones adicionales

Puedes combinar la creación de modelos con otros artefactos relacionados:

<Code code={`# Crear modelo con migración
php artisan make:model Producto -m

# Crear modelo con controlador
php artisan make:model Producto -c

# Crear modelo con controlador resource
php artisan make:model Producto -cr

# Crear modelo con migración, factory y seeder
php artisan make:model Producto -mfs

# Crear modelo con todos los artefactos relacionados
php artisan make:model Producto --all`} lang="bash" />
...
class Producto extends Model
{
    ...
    // Atributos y métodos del modelo...
}`} lang="php" />

### Convenciones de nombres

Eloquent utiliza varias convenciones para determinar la tabla y otras propiedades:

- **Tabla**: Por defecto, Eloquent usará el plural en snake_case del nombre de la clase como nombre de la tabla. Por ejemplo, el modelo <Code code="Producto" lang="php" inline /> corresponderá a la tabla <Code code="productos" lang="sql" inline />.

- **Clave primaria**: Por defecto, Eloquent asume que cada tabla tiene una columna de clave primaria llamada <Code code="id" lang="sql" inline />.

- **Timestamps**: Por defecto, Eloquent espera que existan las columnas <Code code="created_at" lang="sql" inline /> y <Code code="updated_at" lang="sql" inline /> en tus tablas.

### Personalizando convenciones

Puedes personalizar estas convenciones definiendo propiedades en tu modelo:
...
class Producto extends Model
{
    ...
    // Especificar tipo de clave primaria
    protected $keyType = 'string';
    
    // Desactivar timestamps
    public $timestamps = false;
    
    // Personalizar nombres de columnas timestamp
    const CREATED_AT = 'fecha_creacion';
    const UPDATED_AT = 'fecha_actualizacion';
    
    // Especificar conexión a base de datos
    protected $connection = 'mysql_secundaria';
}`} lang="php" />

### Atributos asignables masivamente

Para proteger contra vulnerabilidades de asignación masiva, debes especificar qué atributos pueden ser asignados masivamente usando las propiedades <Code code="$fillable" lang="php" inline /> o <Code code="$guarded" lang="php" inline />:

<Tabs>
<TabItem label="Usando $fillable">

```php
class Producto extends Model
{
    // Solo estos atributos pueden ser asignados masivamente
    protected $fillable = [
...
<TabItem label="Usando $guarded">

```php
class Producto extends Model
{
    // Estos atributos NO pueden ser asignados masivamente
    protected $guarded = [
        'id', 'admin_flag'
    ];
    
    // Para permitir todos los atributos (¡usar con precaución!)
    // protected $guarded = [];
}
```
...
Puedes especificar qué atributos deben ser ocultos o visibles cuando el modelo se convierte a un array o JSON:

<Code code={`class Usuario extends Model
{
    // Atributos que se ocultan al convertir a array/JSON
    protected $hidden = [
        'password', 'remember_token', 'api_key'
    ];
    
    // Atributos que siempre se incluyen al convertir a array/JSON
    protected $visible = [
...
Puedes definir valores predeterminados para atributos usando la propiedad <Code code="$attributes" lang="php" inline />:

<Code code={`class Producto extends Model
{
...
<Aside type="tip">
Cuando creas un modelo con <Code code="php artisan make:model NombreModelo -m" lang="bash" inline />, Laravel también genera una migración para la tabla correspondiente. Esto te permite definir la estructura de la tabla y las relaciones en el mismo momento.
</Aside>
