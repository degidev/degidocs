...
Las claves foráneas son fundamentales en las bases de datos relacionales para mantener la integridad referencial. Laravel facilita la creación y gestión de estas relaciones tanto en migraciones como en modelos Eloquent.

### Definir claves foráneas en migraciones

Hay varias formas de definir claves foráneas en tus migraciones:

#### Método foreignId

El método más común y recomendado en Laravel moderno:

<Code code={`Schema::create('posts', function (Blueprint $table) {
    $table->id();
    $table->string('title');
    $table->text('content');
    $table->foreignId('user_id')->constrained(); // Referencia a la tabla 'users' y columna 'id'
    $table->timestamps();
});`} lang="php" />

El método `constrained()` asume que la tabla referenciada es el plural del nombre de la columna sin el sufijo `_id` y que la columna referenciada es `id`.

#### Personalizar tabla y columna referenciada

<Code code={`// Especificar tabla referenciada
$table->foreignId('author_id')->constrained('users');

// Especificar tabla y columna referenciada
$table->foreignId('author_id')->constrained('users', 'user_id');`} lang="php" />

#### Método tradicional

<Code code={`$table->unsignedBigInteger('user_id');
$table->foreign('user_id')->references('id')->on('users');`} lang="php" />

### Acciones referenciales

Puedes especificar qué debe ocurrir cuando se elimina o actualiza el registro referenciado:

<Code code={`// Eliminar en cascada
$table->foreignId('user_id')
      ->constrained()
      ->onDelete('cascade');
      
// Establecer NULL cuando se elimina el registro referenciado
$table->foreignId('user_id')
      ->constrained()
      ->onDelete('set null');
      
// Restricción (impide eliminar el registro referenciado)
$table->foreignId('user_id')
      ->constrained()
      ->onDelete('restrict');
      
// Acciones para actualización y eliminación
$table->foreignId('user_id')
      ->constrained()
      ->onUpdate('cascade')
      ->onDelete('cascade');`} lang="php" />

### Claves foráneas con nombres personalizados

<Code code={`$table->foreignId('user_id')
      ->constrained()
      ->name('posts_user_id_foreign');`} lang="php" />

### Eliminar claves foráneas

Para eliminar una clave foránea en una migración:

<Code code={`Schema::table('posts', function (Blueprint $table) {
    // Si se especificó un nombre personalizado
    $table->dropForeign('posts_user_id_foreign');
    
    // Si se usó el nombre por convención
    $table->dropForeign(['user_id']);
});`} lang="php" />

### Deshabilitar verificación de claves foráneas

En algunos casos, como al refrescar la base de datos, puede ser necesario deshabilitar temporalmente la verificación de claves foráneas:

<Code code={`Schema::disableForeignKeyConstraints();

// Operaciones que podrían violar restricciones de clave foránea

Schema::enableForeignKeyConstraints();`} lang="php" />

### Relaciones en modelos Eloquent

Las relaciones en Eloquent permiten definir cómo se relacionan los modelos entre sí, facilitando la consulta de datos relacionados.

#### Relación uno a uno

<Code code={`// En el modelo User
...
<Code code={`// Personalizar tabla pivote
$this->belongsToMany(Role::class, 'role_user');

// Personalizar columnas de clave foránea
$this->belongsToMany(Role::class, 'role_user', 'user_id', 'role_id');

// Con columnas pivote adicionales
$this->belongsToMany(Role::class)->withPivot('active', 'created_by');

// Con timestamps en la tabla pivote
$this->belongsToMany(Role::class)->withTimestamps();`} lang="php" />

#### Relaciones polimórficas

<Code code={`// En los modelos Post y Video
...
<Aside type="tip">
  Las relaciones polimórficas permiten que un modelo pertenezca a más de un tipo de modelo. Por ejemplo, un comentario puede pertenecer a un post o a un video.
</Aside>
